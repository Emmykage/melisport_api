class Api::V1::OrderDetailsController < ApplicationController
  ...
  def order_detail_params
    params.require(:order_detail).permit(:total, :viewed, :status, :ref_code, :payment_method, order_items_attributes: [:product_id, :quantity, :amount, {sizes: []}], billing_address_attributes: %i[name email city street state phone_no postal_code ] )
  end
