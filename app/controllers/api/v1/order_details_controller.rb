class Api::V1::OrderDetailsController < ApplicationController
  ...
  def order_detail_params
    params.require(:order_detail).permit(:total, :pick_up, :viewed, :status, :payment_method, :referral_code, :sub_total, :net_total, :agent_id, :paid,
                                         order_items_attributes: [:product_id, :quantity, :amount, :size, :sku, :bonus], billing_address_attributes: %i[name email city street state phone_no postal_code])
  end
end
